...
class PagePlayBottomSheetState extends State<PagePlayBottomSheet> {
  ...
  void _onMusicPlayingStateChange(
    ...
    nLog(
        "PagePlayBottomSheetState _onMusicPlayingStateChange isPlaying : $isPlaying , song : $song");
  ...
  _scrollToPlayingSong() {
    ...
    double scrollDistance = math.min(
        widget.musicPlayerController.songs.length * itemHeight - sheetHeight,
        math.max(
            0, _playingIndex * itemHeight - (sheetHeight - itemHeight) / 2.0));

    _sc.animateTo(scrollDistance,
  ...
  _getDuration(int index) {
    if (index >= 0 && index < widget.musicPlayerController?.songs?.length) {
      final int duration =
          widget.musicPlayerController?.songs[index]["duration"] ~/ 1000;
